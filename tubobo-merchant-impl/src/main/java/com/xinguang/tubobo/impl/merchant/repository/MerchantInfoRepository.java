...
public interface MerchantInfoRepository extends JpaRepository<MerchantInfoEntity, String>, JpaSpecificationExecutor<MerchantInfoEntity> {
    ...
    @Modifying
    @Query("update #{#entityName} a set a.providerId = :providerId, a.enterTime=:enterTime, a.bindStatus=:bindStatus,a.updateDate=:updateDate,a.providerName=:providerName where a.userId = :userId and a.delFlag = :delFlag ")
    int bindProvider(@Param("userId") String userId, @Param("delFlag") String delFlag, @Param("providerId") Long providerId,@Param("providerName") String providerName,@Param("bindStatus") String bindStatus, @Param("enterTime") Date enterTime, @Param("updateDate") Date updateDate);

    @Modifying
    @Query("update #{#entityName} a set a.providerId = :providerId,a.bindStatus=:bindStatus,a.unbundleTime=:unbundleTime, a.updateDate=:updateDate where a.userId = :userId and a.providerId=:oldProviderId and a.delFlag = :delFlag ")
    int unbindProvider(@Param("userId") String userId, @Param("oldProviderId") Long oldProviderId ,@Param("delFlag") String delFlag,@Param("bindStatus") String bindStatus, @Param("providerId") Long providerId, @Param("unbundleTime") Date unbundleTime, @Param("updateDate") Date updateDate);
