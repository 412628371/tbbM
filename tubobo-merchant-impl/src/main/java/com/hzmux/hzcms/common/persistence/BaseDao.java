...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
	...
	@SuppressWarnings("unchecked")
	public <E> Page<E> findPage(String sqlString, Parameter parameter, Class<?> resultClass,int pageNo,int pageSize){
		Page<E> page = new Page<E>(pageNo,pageSize);

		String countSqlString = "select count(*) " + removeSelect(removeOrders(sqlString));
		SQLQuery countQuery = getSession().createSQLQuery(countSqlString);
		setParameter(countQuery, parameter);
		...
		SQLQuery selectQuery = getSession().createSQLQuery(sqlString);
		setParameter(selectQuery, parameter);
		setResultTransformer(selectQuery, resultClass);
		if (pageSize >= 1) {
			selectQuery.setFirstResult((pageNo-1)*pageSize);
			selectQuery.setMaxResults(pageSize);
		}
		page.setList(selectQuery.list());
