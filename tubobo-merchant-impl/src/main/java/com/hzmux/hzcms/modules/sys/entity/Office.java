...
public class Office extends IdEntity<Office> {
	...
	private String type; 	// 机构类型（1：公司；2：部门；3：门店_校园店；4：门店_小区店）
	...
	@Transient
	public static String getFullAddress(Office office){
	    Area area = SpringContextHolder.getBean(AreaService.class).get(office.getArea().getId());
	    return Area.getFullName(area);
	}
    ...
    @Transient
    public static boolean isStore(Office office) {
        if (null != office) {
            return ("3".equals(office.getType()) || "4"
                    .equals(office.getType()));
        } else {
            return false;
        }
    }
